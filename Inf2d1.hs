...
next branch = filter (\z -> (fst (head z)) <=6 && (fst (head z)) >=1 && (snd (head z)) <=6 && (snd (head z)) >=1) --gets rid of out of bounds nodes (off-grid)
	(filter (\y -> notElem (head y) (tail y)) ---gets rid of duplicates
	(filter (\x -> notElem (head x) badNodesList) --gets rid of bad nodes
	[(fst n + 1, snd n):branch, (fst n - 1, snd n):branch, (fst n, snd n + 1):branch, (fst n, snd n - 1):branch])) --appends list
	where
		n = head branch
...
breadthFirstSearch destination next branches exploredList =
	(\z -> if ((checkArrival destination (head z)) Just z ) branches --if a branch in branches contains destination at head return that branch
	if null branches then Nothing
		else then
		map (\x -> next head x) (filter (\y -> notElem (head y) exploredList) branches) --expanding head node of each branch in branches where branches doesnt contain any branches with an explored head node


	--filter (\x -> head x) branches
	--next branch, check each head for destination
	--each head to be checked for destintion will then be added to explored list
	--explored list is checked before destination. if node on explored list then move along
	--keep doing next branch and checking head of each branch for destination
	--if node == destination return (Just Branch)
	--	else return Nothing
	--next branches)

--breadthFirstSearch (2, 4) next [[(1, 1)],[(2,8), (4, 6)]][]
